...
def fetchClips(channel_name,range="7d",max=None):
    ...
    clips = data.findAll("article",{"class": "iGwMza"})
    response = []
    for element in clips:
        response.append(Clip(f"https://www.twitch.tv"+element.find("a",{"data-a-target": "preview-card-image-link"})["href"],element.find("h3",{"class": "dZYafS"})["title"]))
    if max == None:
        return response
    else:
        if max > len(response):
